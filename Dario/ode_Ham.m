function [t,y] = ode_Ham(f,tspan,y0,N,varargin)

% Hamming method to solve vector d.e. y'(t)=f(t,y(t))
% for tspan=[t0,tf] and with the initial value y0 and N time steps using
% the modifier based on the error estimate depending on KC=1/0

% IMPORTANT: the function f must return a column vector (with the first
% order derivatives)
