function fmain_att(workspace_path, verbose, misalignment_type, state_perturbation_flag, engine_failure_flag)
...
% dt_regen = 10/TU;             % renegerative propagation interval
dt_min = 0.5*60/TU;               % minimum propagation interval
prop_step = 1/TU;               % propagation time step
% dt_min = prop_step;               % minimum propagation interval
...
for branch = 1 : max_branches
    ...
    % [tspan_AOCS, Y_AOCS] = ode113(@(t, Y) AOCS_att(t, Y, EarthPPsMCI, SunPPsMCI, muM, ...
    ...
    [tspan_AOCS, Y_AOCS] = odeHamHPC(@(t, Y) AOCS_att(t, Y, EarthPPsMCI, SunPPsMCI, muM, ...
    ...
    for j = 1 : M_AOCS
        ...
        % AOCS Reconstruction
        [~, ~, ~, ~, u_AOCS(j, :), ~, ~, ~, ~, Tc_AOCS(j, :), Ta_AOCS(j, :), xb_AOCS(j, :)] = AOCS_att(tspan_AOCS(j), Y_AOCS(j, :)', EarthPPsMCI, SunPPsMCI, muM, ...
...
for i = 1 : size(RHO_LVLH, 1)
    ...
    elseif i > M_ctrl_DA && i <= M_ctrl_DA + M_ctrl
        ...
        [~, ~, ~, ~, u(i, :), ~, ~, ~, f_norms(i), Tc(s, :), Ta(s, :), ~] = ...
            AOCS_att(tspan_ctrl(s), Y_ctrl(s, :)', EarthPPsMCI, SunPPsMCI, muM, ...
